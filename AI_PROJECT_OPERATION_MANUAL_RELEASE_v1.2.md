...
Prompt는 영구 규칙과 이번 작업을 분리한다.

- 영구 규칙은 운영 매뉴얼이 책임진다.
- 작업 프롬프트는 이번 작업의 목적, 수행 작업, 추가 제한, 보고 형식만 기술한다.
- 작업 프롬프트에는 운영 매뉴얼에 이미 존재하는 규칙을 반복하지 않는다.
...
이 기준은 Prompt 작성 표준이다. AI Contract Protection, Project Authority, Source of Truth, Policy First, Living Contract 원칙을 대체하지 않는다.
