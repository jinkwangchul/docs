# AI Project Operation Manual RELEASE v1.1

## 0. 릴리즈 선언

이 문서는 GitHub 기준 ChatGPT 직접 분석 원칙을 반영한 릴리즈 운영 매뉴얼이다. 당분간 AI 협업 및 프로젝트 운영의 최신 기준으로 사용한다.

기준 문서:

- AI_PROJECT_OPERATION_MANUAL_RELEASE.md

릴리즈 원칙:

- 운영 방식이 크게 바뀌기 전까지 새 운영 매뉴얼 버전을 만들지 않는다.
- 변경이 필요하면 사용자 승인 후 별도 릴리즈 문서를 작성한다.
- 기존 v1.0, v1.1 문서는 삭제하거나 수정하지 않는다.
...
- GitHub: 최신 코드인지, Push되지 않은 로컬 변경이 있는지
- 금지사항: runtime, rules.json, GUI, ExecutionController, SendOrder, Git
- 최소 구현 단위: 이번 작업에서 딱 어디까지 할지
...
이 파일을 당분간 운영 매뉴얼의 최신 기준으로 사용한다. 기존 최소 구현 단위 원칙, Preview -> Approval -> Policy -> Commit -> Review -> Execution -> SendOrder 원칙, GitHub Source of Truth, docs/kiwoom_auto 분리, runtime/rules/GUI/SendOrder 금지선은 유지한다.
